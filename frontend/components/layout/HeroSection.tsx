...
export default function HeroSection({ initial }: { initial?: HeroInitial }) {
  ...
  const slides: HeroSlide[] = (settings?.heroSlides?.length
    ...
    : initial?.heroSlides?.length
      ...
      : []) as HeroSlide[];
  const rotationMs =
    Number(slides[current]?.rotationMs) > 0
      ? Number(slides[current].rotationMs)
      : settings?.defaultBannerRotationMs || initial?.defaultBannerRotationMs || 6000;
  const defaultAnim =
    settings?.defaultHeroAnimation || initial?.defaultHeroAnimation || 'fade';
  ...
  const prefersReducedMotion = useMemo(() => {
    ...
    if (isLegacyStorefrontDevice()) return true;
        ...
        */}
        ...
        >
          {slides.map((s, i) => (
            <div
              key={i}
              className={`ob-hero-slide${prefersReducedMotion ? '' : ` ob-hero-anim-${String(s.animation || defaultAnim).replace(/[^a-z0-9-]/gi, '')}`}`}
              data-ob-hero-animation={prefersReducedMotion ? 'none' : s.animation || defaultAnim}
              style={{
                position: 'relative',
                flex: '0 0 100%',
                width: '100%',
