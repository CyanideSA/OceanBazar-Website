...
export default function AddressBookPanel() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['addresses'],
    queryFn: () => profileApi.addresses().then((r) => r.data.addresses as SavedAddress[]),
  });
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-foreground">
                    {addr.label}
                    {addr.isDefault ? (
                      <span className="ml-2 text-xs font-normal text-primary">({t('addrDefault')})</span>
                    ) : null}
                  </p>
                ...
                <div className="flex shrink-0 gap-1">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (typeof window !== 'undefined' && window.confirm(tc('confirmDelete')))
