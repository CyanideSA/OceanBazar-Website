...
export type ProductCollectionPageClientProps = {
  collection: string;
  initialData?: ProductsListResponse | null;
...
function ProductCollectionPageInner({
  ...
  const collection = collectionProp || String(params.collection || '').trim();
  ...
  const seeded =
    ...
    sort === (config.sort ?? 'createdAt_desc');
  ...
  const { data, isLoading } = useQuery({
    ...
    initialData: seeded ? (initialData ?? undefined) : undefined,
    staleTime: 60_000,
  });

  const products: Product[] = data?.products ?? (seeded ? initialData?.products : undefined) ?? [];
  const categories: Category[] = catData?.categories ?? initialCategories ?? [];
  const pagination = data?.pagination ?? (seeded ? initialData?.pagination : undefined);
  const pageCount = pagination?.pages ?? 0;


  return (
    <div
      className="container-tight py-5 sm:py-8"
      data-ob-ssr-products={String(products.length)}
      data-ob-collection={collection}
    >
...
export default function ProductCollectionPageClient(props: ProductCollectionPageClientProps) {
  return (
    <Suspense fallback={<ProductGridSkeleton />}>
      <ProductCollectionPageInner {...props} />
    </Suspense>
  );
}
