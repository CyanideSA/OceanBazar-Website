...
export type ProductsPageClientProps = {
  ...
  initialFiltersData?: FiltersData | null;
};

function ProductsPageInner({ initialData, initialFiltersData }: ProductsPageClientProps) {
  ...
  const { data: filtersData } = useQuery({
    ...
    staleTime: 5 * 60 * 1000,
    initialData: initialFiltersData ?? undefined,
  });
  ...
  const trustBadge = (searchParams.get('trustBadge') || '').trim();
  ...
  const seededFirstPage =
    Boolean(initialData) &&
    page === 1 &&
    search === urlSearch &&
    filters.category === (searchParams.get('category') ?? '') &&
    filters.brands.join(',') === (searchParams.get('brand') ?? '') &&
  ...
  const { data, isLoading, isFetching, isError } = useQuery({
    ...
    initialData: seededFirstPage ? (initialData ?? undefined) : undefined,
    staleTime: 60_000,
  });

  const products: Product[] = data?.products ?? (seededFirstPage ? initialData?.products : undefined) ?? [];
  const pagination = data?.pagination ?? (seededFirstPage ? initialData?.pagination : undefined);
  ...
  return (
    <div
      className="container-tight relative py-5 sm:py-8"
