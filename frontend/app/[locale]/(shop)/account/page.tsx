...
export default function AccountDashboardPage() {
  ...
  const saveProfile = useMutation({
    ...
    onSuccess: (res) => {
      ...
      if (raw) {
        const u = mapProfileUser(raw);
        updateUser(u);
        setPhotoUrl(u.profileImage ?? '');
      }
          ...
          <div className="mt-4 space-y-2 border-t border-border pt-4 sm:mt-6 sm:pt-6">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              <button
                ...
                className="rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground disabled:opacity-50 min-h-[44px] sm:w-auto"
              >
              ...
              {photoUrl ? (
                <button
                  ...
                  onClick={() => {
                    saveProfile.mutate({ profileImage: null });
                    setPhotoUrl('');
                  }}
                  className="rounded-lg border border-border px-4 py-2.5 text-sm font-semibold text-muted-foreground hover:bg-accent min-h-[44px]"
                >
                  {t('removePhoto')}
                </button>
              ) : null}
