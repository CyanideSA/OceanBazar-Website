...
import {
  ...
  Building2, Users, MapPin, Clock, CheckCircle2, Send, Mail, MessageCircle,
} from 'lucide-react';
import { businessInquiriesApi, storefrontApi } from '@/lib/api';
...
export default function BusinessInquiriesClient({
  initialSettings,
}: {
  initialSettings?: StorefrontPublicSettings;
}) {
  const locale = useLocale();
  const tRaw = useTranslations('business');
  ...
  const settings = coalesceStorefrontSettings(remoteSettings, initialSettings);
  const ov = useMemo(
    () => getMessageOverrides(settings.pageContent, 'businessInquiries', locale),
  ...
  const businessEmail = String(settings?.businessInquiryEmail || 'business@oceanbazar.com.bd').trim();
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitError('');
    try {
      await businessInquiriesApi.submit({
      ...
      <div className="mb-12 overflow-hidden rounded-2xl bg-gradient-to-br from-primary/90 to-primary px-8 py-14 text-primary-foreground shadow-soft md:px-14">
        ...
        {businessEmail ? (
          <a
            ...
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-white/15 px-4 py-2.5 text-sm font-semibold backdrop-blur-sm transition hover:bg-white/25"
          >
            ...
            <div className="flex flex-col justify-center p-8 md:p-10">
              ...
              {businessEmail ? (
                <div className="mt-5 rounded-xl border border-border bg-card p-4">
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t('inquiryEmailLabel')}</p>
                  <a href={`mailto:${businessEmail}`} className="mt-1 inline-flex items-center gap-2 text-sm font-bold text-primary hover:underline">
                    <Mail className="h-4 w-4" />
                    {businessEmail}
                  </a>
                </div>
            ...
            <div className="border-t border-primary/10 p-8 lg:border-l lg:border-t-0 md:p-10">
              ...
              {sent ? (
                <div className="flex flex-col items-center gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-8 text-center">
                  ...
                  {businessEmail ? (
                    <a href={`mailto:${businessEmail}`} className="text-sm text-primary hover:underline">
                      {businessEmail}
                    </a>
                  ) : null}
                </div>
                ...
                <form onSubmit={onSubmit} className="space-y-3.5">
                  ...
                  {submitError ? (
                    <p className="rounded-xl border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-sm text-rose-700 dark:text-rose-300">
                      {submitError}
                    </p>
                  ) : null}
                  <button
                    type="submit"
                    disabled={submitting}
                    className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 font-bold text-primary-foreground shadow-soft transition-all hover:brightness-110 hover:shadow-glow-primary active:scale-[0.98] disabled:opacity-60"
                  >
