...
export type HomeSectionKey = 'featured' | 'bestDeals' | 'latest' | 'trending' | 'bestRated' | 'mostSold';
...
export type HomeCatalog = {
  ...
  siteSettings?: StorefrontPublicSettings;
};
...
export async function fetchHomeCatalog(locale: string): Promise<HomeCatalog> {
  ...
  const [sectionResults, flash, categories, brandsPayload, settings] = await Promise.all([
    ...
    fetchStorefrontJson<StorefrontPublicSettings>(
  ...
  return {
    ...
    heroSlides: Array.isArray(settings?.heroSlides)
      ? (settings.heroSlides as Array<Record<string, unknown>>)
      : [],
    defaultBannerRotationMs: Number(settings?.defaultBannerRotationMs) || 6000,
    siteSettings: settings && typeof settings === 'object' ? settings : {},
  };
}
