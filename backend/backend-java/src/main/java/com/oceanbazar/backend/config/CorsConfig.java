...
public class CorsConfig {
    ...
    public CorsFilter corsFilter() {
        ...
        String rawPatterns = environment.getProperty(
                "app.cors.allowed-origin-patterns",
                "http://localhost:*,http://127.0.0.1:*,https://*.vercel.app,https://*.onrender.com"
        );
        List<String> allowedPatterns = Arrays.stream(rawPatterns.split(","))
                .map(String::trim)
                .filter(s -> !s.isBlank())
                .collect(Collectors.toList());
