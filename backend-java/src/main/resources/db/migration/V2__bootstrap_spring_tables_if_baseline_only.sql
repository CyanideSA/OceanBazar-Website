-- If the database was baselined at version 1 without running V1 (empty schema_history only),
-- Flyway skips V1 and Spring-owned tables never get created. This migration repeats the same
-- idempotent DDL so those environments get the tables on upgrade to V2.
-- (Same statements as V1__spring_boot_tables.sql.)
