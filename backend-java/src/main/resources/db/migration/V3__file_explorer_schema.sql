...
-- When the DB is owned by Prisma first, public.categories already has tree columns (e.g. path);
-- running the legacy rename + CREATE TABLE path would destroy that schema — skip in that case.
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'Flyway V3 skipped: public.categories already has explorer columns (Prisma-compatible schema).';
  ELSE
